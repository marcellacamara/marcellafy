...
<div class="flex bg-transparent hover:bg-gray-900/10 rounded-lg border border-gray-900/10 shadow-md">
  ...
  <img class="shadow rounded w-24 align-middle border-none rounded-t-lg" src="{{ asset($album->cover_image) }}"
    alt="{{ $album->title }}" style="width: 8rem; height: 8rem;object-fit: cover;" />
  <div class="flex flex-1 flex-col justify-center items-center">

    <span class="text-2xl font-bold uppercase tracking-tight text-white">{{ $album->title }}</span>
